...
/**
 * This class creates a Frigate battleship.
 */
public class FrigateLayout extends ShipLayout
{
	static final int NUMBER_OF_POINTS_FOR_FRIGATE = 3;
	...
	/*
	 * Constructor for the frigate
	 */
	public FrigateLayout(Point3D referencePoint, int shipOrientation, int xDimension, int yDimension, int zDimension)
	{
		super(ServerApi.FRIGATE, referencePoint, NUMBER_OF_POINTS_FOR_FRIGATE, shipOrientation);
	...
	/*
	 * Creates the layout for the frigate ship that will fit inside the gameboard. Ensures
	 * that the direction that the points expand from the reference are valid.
	 */
	public void createFrigateShipLayout(Vector<Point3D> usedPoints)
	{
		...
		if(X_EXPANDING == shipOrientation_)
		{
			...
			for(int i = 1; i < NUMBER_OF_POINTS_FOR_FRIGATE; i++)
			{
		...
		{
			for(int i = 0; i < NUMBER_OF_POINTS_FOR_FRIGATE; i++)
			{
	...
	private void validateShip(Vector<Point3D> usedPoints)
	{
		...
		{
			for(int j = 0; j < NUMBER_OF_POINTS_FOR_FRIGATE; j++)
			{
